    ...
    override fun get(
        ...
        return buildList {
            getList.forEachIndexed { index, line ->
                ...
                val type = PotionEffectType.getByName(split[0]) ?: return@forEachIndexed config.nullError("$path:$index", "Potion(${split[0]})")
                val duration = split[1].toIntOrNull() ?: return@forEachIndexed config.typeMismatchError("$path:$index", "Int(${split[1]})")
                val level = split[2].toIntOrNull() ?: return@forEachIndexed config.typeMismatchError("$path:$index", "Int(${split[2]})")
