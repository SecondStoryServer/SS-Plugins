    ...
    override fun onEnable() {
        ...
        command(plugin,
            ...
            when (args.whenIndex(0)) {
                "load" -> {
                    val pluginName = args.getOrNull(1) ?: return@command sendError("プラグイン名を入力してください")
                    if (pluginManager.getPlugin(pluginName) != null) return@command sendError("読み込まれているプラグインです")
                    val file = pluginsFolder.getPluginJar(pluginName) ?: return@command sendError("プラグインが見つかりませんでした")
                ...
                "enable" -> {
                    val pluginName = args.getOrNull(1) ?: return@command sendError("プラグイン名を入力してください")
                    val plugin = pluginManager.getPlugin(pluginName) ?: return@command sendError("存在しないプラグインです")
                    if (plugin.isEnabled) return@command sendError("既に有効化されています")
